		...
		if(isset($_POST['Nama_Depan'])){		
			if($_POST['Nama_Depan'] != "" && $_POST['Nama_Belakang'] != "" && $_POST['Email'] != "" && $_POST['Password'] != "" && $_POST['Nomor_Telepon'] != ""){
				...
				if(!$hasilQuery -> num_rows > 0){
					$daftarQuery = "INSERT INTO akun values ('" . $_POST['Nama_Depan'] . "','" . $_POST['Nama_Belakang'] . "','" . $_POST['Email'] . "','" . $_POST['Password'] . "','" . $_POST['Nomor_Telepon'] . "','Guru')";
					$terdaftar = $Koneksi -> query($daftarQuery);				
					
					if($terdaftar){
						$_SESSION['user'] = $_POST['Email'];
						echo "<script>window.location.assign('/Page Guru');</script>";
					}
				}else{
		...
		if(isset($_POST['EmailIn'])){		
			if($_POST['EmailIn'] != "" && $_POST['PasswordIn'] != ""){				
				if (filter_var($_POST['EmailIn'], FILTER_VALIDATE_EMAIL)) {
					$query = "SELECT * FROM akun WHERE Email = '". $_POST['EmailIn'] ."' AND Jenis_Akun = 'Guru'";
